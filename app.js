...
function renderClients() {
  ...
  content.innerHTML = `<div class="intro"><h2>Clientes</h2><p>Clique em um cliente para abrir o resumo comercial atualizado.</p></div><div class="panel table-wrap"><table><thead><tr><th>Empresa</th><th>Alerta</th><th>Cidade</th><th>Região</th><th>Última atividade</th></tr></thead><tbody>${state.clients.map((item, index) => { const info = alertInfo(monthAge(item.lastDate)); return `<tr><td><button class="link-button" data-client="${index}">${escapeHtml(item.name)}</button></td><td><span class="status ${info.css}">${info.label}</span></td><td>${escapeHtml(item.city || "-")}</td><td>${escapeHtml(item.region || "-")}</td><td>${escapeHtml(item.lastDate || "Não informado")}</td></tr>`; }).join("") || '<tr><td colspan="5" class="empty">Importe sua planilha padrão para começar.</td></tr>'}</tbody></table></div>`;
