...
async function createCalendarEvent(client) {
  ...
  const event = {
    ...
    description: `CNPJ: ${client.cnpj || "não informado"}\nVendas: ${client.sales}\nOrçamentos: ${client.quotes}`,
...
const rebuildClientSummary = records => {
  ...
  records.forEach(record => {
    ...
    if (!key) return;
    const current = clients.get(key) || { name: "", cnpj: "", city: "", state: "", region: "", lastDate: "", sales: 0, quotes: 0, contacts: 0, total: 0 };
...
function renderDashboard() {
  ...
  const alertCounts = ["green", "yellow", "red"].map(css => state.clients.filter(item => alertInfo(monthAge(item.lastDate)).css === css).length);
  const recentClients = [...state.clients].sort((a, b) => (b.lastDate || "").localeCompare(a.lastDate || "")).slice(0, 5);
  content.innerHTML = `<div class="dashboard-hero"><div><span class="eyebrow">PAINEL COMERCIAL</span><h2>Bom dia, equipe!</h2><p>Acompanhe sua carteira e priorize os próximos contatos.</p></div><button class="primary-button" data-dashboard-view="clients">Ver clientes</button></div>
    <div class="cards dashboard-cards"><div class="card accent-blue"><span class="card-icon">◆</span><small>Clientes na base</small><strong>${state.clients.length.toLocaleString("pt-BR")}</strong><span class="card-caption">Empresas acompanhadas</span></div>
...
function renderClients() {
  ...
  content.innerHTML = `<div class="intro"><h2>Clientes</h2><p>Clique em um cliente para abrir o resumo comercial atualizado.</p></div><div class="panel table-wrap"><table><thead><tr><th>Empresa</th><th>CNPJ</th><th>Cidade</th><th>Região</th><th>Última atividade</th><th>Alerta</th></tr></thead><tbody>${state.clients.map((item, index) => { const info = alertInfo(monthAge(item.lastDate)); return `<tr><td><button class="link-button" data-client="${index}">${escapeHtml(item.name)}</button></td><td>${escapeHtml(item.cnpj || "-")}</td><td>${escapeHtml(item.city || "-")}</td><td>${escapeHtml(item.region || "-")}</td><td>${escapeHtml(item.lastDate || "Não informado")}</td><td><span class="status ${info.css}">${info.label}</span></td></tr>`; }).join("") || '<tr><td colspan="6" class="empty">Importe sua planilha padrão para começar.</td></tr>'}</tbody></table></div>`;
